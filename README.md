...
Node.js 20 이상을 설치한 뒤 이 폴더에서 실행합니다. 외부 npm 패키지는 필요 없습니다.
...
보유한 포트폴리오 이미지로 교체하려면 `source/manifest.json`에서 원본 URL 또는 파일명을 검색한 뒤, 대응하는 `source/files/` 파일을 같은 형식·파일명으로 바꾸고 다시 빌드합니다. 이미지 비율을 유지하면 원본 레이아웃이 유지됩니다. 상세 이미지 속 문구는 이 프로젝트에서 재작성하지 않았습니다.
