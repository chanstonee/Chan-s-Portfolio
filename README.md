...
실제 페이지·지연 로딩·소셜 미리보기에서 쓰는 이미지 86개를 보존하고, 참조가 전혀 없는 이미지 9개를 제거했습니다. 공통 이미지는 `avatar.png`, `home-tv-frame.png`처럼 용도를 이름에 적었고, 프로젝트 이미지는 `dssystem-01.jpg`, `playon-01.jpg`처럼 프로젝트별 순번으로 정리했습니다.
